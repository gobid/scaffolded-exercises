 ...
 * is a limitation/issue to eventually address... @TODO
 */
let sourceCodeMap = {};
...
function enter(node) {
    ...
    if (isFunctionDeclaration(node)) {
        ...
        sourceCodeMap[fxnName] = { start: node.loc.start, end: node.loc.end };
        const updateStr = `\n/* autogen added */ \n ${fxnName} = StackTrace.instrument(${fxnName}, fxnCallCallback(\`${fxnName}\`, sourceCodeArr, sourceCodeMap));\n/* end autogen added */\n`;
    ...
    /** @TODO / WiP: if a method is called when assigning a value to a variable, it also should be traced.
     ...
     */
    if (isRHSMethod(node)) {
        console.log("Right hand side method identified. TODO.");
    }
