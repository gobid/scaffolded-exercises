...
const stateManagerStr = fs
    .readFileSync(`./state_manager_${fileKey}.json`)
    .toString();
...
for (let fxn of scopeList) {
    ...
    if (functionMappings[fxnName]) {
        console.log(
            `FUNCTION MAPPING ERROR: Function ${fxnName} exists in multiple scopes`
        );
    } else {
    ...
    esprima.parseScript(source, {}, function (node, meta) {
        if (!updates) {
            ...
            if (
                ...
                !isUpdateAdded(source, meta.end.offset)
            ) {
                ...
                source =
                    source.slice(0, endLoc + 1) +
                    updateStr +
                    source.slice(endLoc + 1);
                updates = true;
...
function isVariableUpdate(node) {
    ...
    if (node.left !== undefined) {
        if (
            ...
            node.left.object.name === "stateManager"
        ) {
            return false;
        }
    }
...
function getAssignmentValue(assignment, nodeType) {
    ...
    switch (nodeType) {
        ...
        default:
            spprint(
                "getAssignmentValue",
                `node val type ${nodeType} not handled`
            );
    }
