...
function eventPos(e) {
    if (e.type.match(/^touch/)) {
        e = e.originalEvent.changedTouches[0];
        /* autogen added */
        updateStateManager("Program:eventPos:e", e);
        /* end autogen added */
    }
...
function Map($container) {
    ...
    var $overlay = $container.children("img");
    /* autogen added */
    updateStateManager("Program:Map:$overlay", $overlay);
    /* end autogen added */
    ...
    StackTrace.instrument(() => {
        $overlay.css({ background: "transparent", position: "relative" });
    ...
    var offset = $container.offset();
    /* autogen added */
    updateStateManager("Program:Map:offset", offset);
    /* end autogen added */
    ...
    var padding_top = 200;
    /* autogen added */
    updateStateManager("Program:Map:padding_top", padding_top);
    /* end autogen added */
    ...
    var size = [14, 48, 25, 33];
    /* autogen added */
    updateStateManager("Program:Map:size", size);
    /* end autogen added */
    ...
    var tilesize = 2048;
    /* autogen added */
    updateStateManager("Program:Map:tilesize", tilesize);
    /* end autogen added */
    ...
    var visible = [];
    /* autogen added */
    updateStateManager("Program:Map:visible", visible);
    /* end autogen added */
    ...
    var container_size = [$container.width(), $container.height()];
    /* autogen added */
    updateStateManager("Program:Map:container_size", container_size);
    /* end autogen added */
    ...
    /* autogen added */
    updateStateManager("Program:Map:scroll_delta", scroll_delta);
    /* end autogen added */
    ...
    var $map = $container.children(".map");
    /* autogen added */
    updateStateManager("Program:Map:$map", $map);
    /* end autogen added */
    ...
    var map_size = [(size[1] + size[3]) * tilesize, (size[0] + size[2]) * tilesize];
    /* autogen added */
    updateStateManager("Program:Map:map_size", map_size);
    /* end autogen added */
    ...
    StackTrace.instrument(() => {
        $map.css({ width: map_size[0], height: map_size[1], position: "absolute", zIndex: -1 });
    }, fxnCallCallback(`$map.css({`, sourceCodeArr, sourceCodeMap))();
    /* end autogen added */ var position = [-(size[3] + 0.03) * tilesize, -(size[0] - 0.55) * tilesize];
    /* autogen added */
    updateStateManager("Program:Map:position", position);
    /* end autogen added */
    ...
    /* end autogen added */ var centre = [-1, 0];
    /* autogen added */
    updateStateManager("Program:Map:centre", centre);
    /* end autogen added */
    ...
    function update() {
        ...
        StackTrace.instrument(() => {
            $map.css({ left: position[0], top: position[1] });
        }, fxnCallCallback(`$map.css({`, sourceCodeArr, sourceCodeMap))();
        /* end autogen added */ var centre_last = centre;
        /* autogen added */
        updateStateManager("Program:Map:update:centre_last", centre_last);
        /* end autogen added */
        ...
        centre = [Math.floor(-position[0] / tilesize), Math.floor(-position[1] / tilesize)];
        /* autogen added */
        updateStateManager("Program:Map:update:centre", centre);
        /* end autogen added */
        ...
        function tile_name(x, y) {
            x -= size[3];
            /* autogen added */
            updateStateManager("Program:Map:update:tile_name:x", x);
            /* end autogen added */
            y -= size[0];
            /* autogen added */
            updateStateManager("Program:Map:update:tile_name:y", y);
            /* end autogen added */
            return (y >= 0 ? y + 1 + "s" : -y + "n") + (x >= 0 ? x + 1 + "e" : -x + "w");
        }
        if (centre[0] != centre_last[0] || centre[1] != centre_last[1]) {
            ...
            var $remove = $map.children().not(".ground");
            /* autogen added */
            updateStateManager("Program:Map:update:$remove", $remove);
            /* end autogen added */
            ...
            for (var y = -1; y <= +1; y++) {
                /* autogen added */
                updateStateManager("Program:Map:update:y", y);
                /* end autogen added */
                ...
                for (var x = -1; x <= +1; x++) {
                    /* autogen added */
                    updateStateManager("Program:Map:update:x", x);
                    /* end autogen added */
                    ...
                    /* autogen added */
                    updateStateManager("Program:Map:update:name", name);
                    /* end autogen added */
                    ...
                    var tile = $map.find(".tile" + name);
                    /* autogen added */
                    updateStateManager("Program:Map:update:tile", tile);
                    /* end autogen added */
                    ...
                    if (tile.length) {
                        ...
                        /* autogen added */
                        updateStateManager("Program:Map:update:$image", $image);
                        /* end autogen added */
    ...
    function drag(e) {
        if (scroll_delta) {
            var pos = eventPos(e);
            /* autogen added */
            updateStateManager("Program:Map:drag:pos", pos);
            /* end autogen added */
            ...
            /* autogen added */
            updateStateManager("Program:Map:drag:position", position);
            /* end autogen added */
            position[1] = Math.round(
                clamp(pos.pageY + scroll_delta[1], -(size[0] + size[2]) * tilesize + container_size[1], 0)
            );
    ...
    StackTrace.instrument(() => {
        $container.on("mousedown touchstart", function (e) {
            ...
            var pos = eventPos(e);
            /* autogen added */
            updateStateManager("Program:Map:pos", pos);
            /* end autogen added */
            ...
            StackTrace.instrument(() => {
                ...
                /* end autogen added */ e.preventDefault();
    ...
    StackTrace.instrument(() => {
        $(document).on("mouseup touchend", function (e) {
            ...
            /* end autogen added */ scroll_delta = null;
            /* autogen added */
            updateStateManager("Program:Map:scroll_delta", scroll_delta);
            /* end autogen added */
        });
...
/* 50:72:6f:50:75:6b:65:20:69:73:20:61:77:65:73:6f:6d:65 */ $(function () {
    var map = new Map($("#comic"));
    /* autogen added */
    updateStateManager("Program:map", map);
    /* end autogen added */
